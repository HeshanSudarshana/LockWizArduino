...
void loop() {
  ...
  if(intruder == 1) {
      ...
      digitalWrite(ledPinSuccess, HIGH); //failure led on
      buzzerOn(); //buzzer on
      Serial.println("intruder detected!!");
  }

  if((!client) && (sensorState == 1)) {
    return;
  }
  ...
  if(!client){
    if(sensorState == 0){
      if(lockValue == 1) {
        ...
        digitalWrite(ledPinSuccess, LOW); //failure led off
